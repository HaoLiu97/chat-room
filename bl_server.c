// Implement the server_actual which manages the interactions between clients in this file making use
...
void ping_clients(int sig) {
    ...
    alarm(1);
    server_tick(server);
    dbg_printf("server_actual has ran for %d second.\n", server_actual.time_sec);
    server_ping_clients(server);
}

int main(int argc, char *argv[]) {
    if(argc <= 1) {
        log_printf("Please specify the server_actual name.\n");
        return 0;
    }
    ...
    // The server_actual should handle SIGTERM and SIGINT by shutting down gracefully.
    struct sigaction sa;
    ...
    while(1) {
        ...
        if (server_join_ready(server)) {
            ...
            dbg_printf("new client join server_actual.\n");
        }

        // handle data sent from each client
        for (int i = 0; i < server_actual.n_clients; i++) {
